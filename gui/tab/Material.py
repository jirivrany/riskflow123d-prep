...
class MaterialTab(QWidget, Ui_tab_material):
    ...
    def get_material_from_dict(self):
        ...
        try:
            ...
            self.window().statusBar.set_message('ERROR - no such material : {}'.format(idx), 8000)
        else:
            #type and type spec (conductivity)
            self.edit_type.setText(material['type'])
            self.edit_specific_data.setText(str(material['type_spec'][0]))
            #geometry
            self.edit_geometry_type.setDisabled(True)
            if material['type'] == '21':
                self.edit_geometry_coeficient.setEnabled(True)
                self.edit_geometry_type.setText(str(int(material['geometry'][0])))
                self.edit_geometry_coeficient.setText(str(material['geometry'][1]))
            else:
                self.edit_geometry_coeficient.setDisabled(True)
            #storativity
            self.edit_storativity.setText(str(material['storativity'][0]))
            #dual porosity
            self.editl_dual_porosity.setText(str(material['dualporosity'][0]))     
